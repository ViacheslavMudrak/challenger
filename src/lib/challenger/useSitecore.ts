...
export const useSitecore = () => {
  ...
  /**
   * The visible request path (e.g. '/', '/individual', '/adviser').
   *
   * Uses `router.asPath` because in Content SDK the page object no longer exposes a top-level
   * `itemPath` (it now lives at `page.layout.sitecore.context.itemPath`), and `asPath` is both
   * SSR-safe and already normalised of locale/basePath and multisite/personalize rewrites.
   */
