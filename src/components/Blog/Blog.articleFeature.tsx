...
const BlogArticleFeature = (props: BlogArticleProps): React.JSX.Element => {
  const {
    image_url,
    article_description,
    article_link = '',
  ...
  return (
    ...
    >
      <div
        className={'relative flex h-[235px] w-full md:h-[395px] xl:h-full xl:w-[435px] xl:flex-row'}
      >
        <div
          className={classNames(
      ...
      <div
        className={
          'relative flex min-h-[300px] w-full flex-col justify-between p-6 py-4 xl:h-full xl:w-[445px] xl:px-10 xl:py-6'
        }
      >
              ...
              categoryList.slice(0, 2).map((item) => {
                return (
                  <a href={getFullLink(parentLink, item)} title={item} key={item}>
                    <span className="mb-4 block rounded-sm border-2 border-challenger-green p-1 !px-2 text-center font-roboto-700 text-bright-navy">
                      {item}
                    </span>
                  </a>
                );
