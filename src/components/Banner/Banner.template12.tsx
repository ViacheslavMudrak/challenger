...
import { BannerArticleSummary } from './Banner.articleSummary';
...
const BannerTemplate12 = (props: BannerComponentProps) => {
  const { BannerImage, bgColorClass, BannerSubHeading } = props?.rendering?.fields;
  ...
  const renderSubHeading = () => {
    return (
      <RichText
        tag="span"
        article_category={BannerSubHeading?.value}
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          <BannerArticleSummary {...props?.rendering?.fields} TextColor={textColour} />
        </div>
      </div>
    </div>
  );
...
export default BannerTemplate12;
